...
app <- rtweet_app(bearer_token = bearer_tokenm)
auth_as(app)
...
parlament_users=lookup_users(parlamento$Twitter)



parltweet=get_timeline(parlament_users$id_str, n=50, retryonratelimit = TRUE)
...
parltweetcsv <- apply(parltweet,2,as.character)
try(write.csv(parltweetcsv, file =  paste(today("CEST"),".csv")))
parltweet$screen_name=0
aad=lookup_tweets(parltweet$id_str)
aa=users_data(aad) 
parltweet$screen_name=aa$screen_name
...
>>>>>>> parent of a9d1c1ae4 (rtweet fixed maybe)
